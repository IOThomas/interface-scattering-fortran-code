...
submodule (io_variables) io_real
    implicit none
contains

    module procedure assign_value_real
        this%value = value
    end 

    module procedure write_out_fr
        character(len=30) :: format
        
        if (dtv%units=="") then 
            write(format, '(a, i2, a,i2, a)') '(a', vlist(1), ',a,es',vlist(2),'.2)'
            write(unit, fmt=format, iostat=iostat) dtv%name,': ',dtv%value
        else if (dtv%name=="") then
            write(format, '(a, i2, a,i2, a)') '(es', vlist(2), '.2,a, a',vlist(3),')'
            write(unit, fmt=format, iostat=iostat) dtv%value, ' ', dtv%units
        else if ((dtv%name=="").and.(dtv%units=="")) then
            write(format, '(a, i2,a)') '(es',vlist(2),'.2)'
            write(unit, fmt=format, iostat=iostat) dtv%value
        else 
            write(format, '(a,i2,a,i2, a,i2,a)') '(a', vlist(1),'a,es',vlist(2),'.2,a, a', vlist(3),')'
            write(unit, fmt=format, iostat=iostat) dtv%name, ': ',dtv%value, ' ', dtv%units
        end if
    end

    module procedure write_out_ur
